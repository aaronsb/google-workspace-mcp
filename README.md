...
Those 80 operations reach 35 of the 233 methods Google publishes across these seven APIs. The rest is mapped, not guessed — see **[API coverage](docs/coverage.md)**, which is generated by diffing the manifest against Google's live Discovery documents.
...
```
 ...
 Google Discovery documents                        MCP Client
 (the machine-readable API spec)                       │ stdio
          │                                            ▼
          ▼                                     ┌──────────────┐
 ┌──────────────────────┐                       │   Factory    │◀── manifest/*.yaml
 │ descriptor generator │                       │  generator   │   (which ops to expose)
 │  scripts/generate-   │                       └──────┬───────┘
 │    descriptor.mjs    │                              │ schemas + handlers
 └──────────┬───────────┘                              ▼
            │                                   ┌──────────────┐
            ▼                                   │   Patches    │  Gmail hydration,
   src/google/descriptor.json ──────────────▶   │              │  agenda merging,
   233 methods · committed                      └──────┬───────┘  MIME, formatting
   paths · verbs · params · scopes                     │
                                                       ▼
                                                ┌──────────────┐
                                                │ Google client│──▶ Google REST APIs
                                                │ + account    │    (call / upload /
                                                │   router     │     download)
                                                └──────────────┘
```
...
This server originally dispatched through [`gws`](https://github.com/googleworkspace/cli), a Rust CLI for the Workspace APIs. It was a good fit at the time: it was fresh, it covered a lot of surface quickly, and it let the manifest-driven factory take shape without also owning a transport. The factory model came from that era and remains the core of the design.

The CLI lives in Google's GitHub organization, but it is not a supported Google product, and it is not actively maintained. More importantly for us, it had **opinions**. It invented its own response shapes, and some of its conveniences were subtly wrong — one helper the server depended on had never worked in any release, which our tests could not see because they mocked the subprocess rather than running the binary. A wrapper that reshapes responses puts a second, unversioned API between you and the one you actually target.

So the server now generates its client from Google's Discovery documents and calls Google directly. This removed a ~19 MB platform-specific binary, halved the bundle, deleted the subprocess boundary, and made the API specification something we can diff against Google on every build. The coverage mapper survived the move — retargeted from scraping a CLI's help text to reading Google's real surface — so growing the toolset is still a manifest edit.

The full decision, including what was verified and what it cost, is in **[ADR-103](docs/architecture/core/ADR-103-generate-a-google-api-descriptor-retire-the-gws-facade.md)**.
