...
version-stamp: ## Write version from package.json into src/version.ts
	@node -e "const v=require('./package.json').version; \
		const fs=require('fs'); \
		fs.writeFileSync('src/version.ts', '// Auto-generated by \`make version-stamp\`. Do not edit manually.\nexport const VERSION = \''+v+'\';\n'); \
		console.log('  src/version.ts → '+v)"

lint: ## Lint src/
	npm run lint

# Delegates rather than re-implementing `tsc && cp`: the npm script carries a
# postbuild integrity check, and a copy of the recipe here would silently skip it.
build: version-stamp ## Compile TypeScript to build/ (and verify the output)
	npm run build

check-gates: ## Assert every test file is COLLECTED by some gate
	node scripts/check-test-gates.mjs

smoke: ## Start the built server on a foreign cwd and assert it loads its tools
